...
def ensemble_scaling(logit,label,loss,t,n_class, v=False):
    ...
    if loss == 'ce':
        w = optimize.minimize(ll_w, (1.0, 0.0, 0.0) , args = (p0,p1,p2,label), method='SLSQP', constraints = constraints, bounds=bnds_w, tol=1e-12, options={'disp': v})
    if loss == 'mse':
        w = optimize.minimize(mse_w, (1.0, 0.0, 0.0) , args = (p0,p1,p2,label), method='SLSQP', constraints = constraints, bounds=bnds_w, tol=1e-12, options={'disp': v})
    w = w.x
    return w
...
def ets_calibrate(logit,label,logit_eval,n_class,loss='mse', v=False):
    t = temperature_scaling(logit,label,loss=loss) # loss can change to 'ce'
    if v:
        print("temperature = " +str(t))
    w = ensemble_scaling(logit,label,loss,t,n_class, v=v)
    if v:
        print("weight = " +str(w))
